...
export default {
  ...
  created() {
    // ⭐ TRIAL: ไม่ fetch real activations — โหลด demo content แล้ว inject เป็น fake activation
    // (trial user ไม่มี activation entry ใน DB — flag-based demoExpiresAt)
    this._loadTrialContent()
  ...
  methods: {
    ...
    // ⭐ TRIAL: โหลด demo content จาก /api/me/trial-content แล้ว inject เป็น fake activation
    async _loadTrialContent() {
      this.activationStore.loading = true
      try {
        const [state, content] = await Promise.all([
          api.get('/me/state').catch(() => null),
          api.get('/me/trial-content').catch(() => null)
        ])

        if (!content?.sections?.length) {
          this.activationStore.activations = []
          return
        }

        const expiresAt = state?.demoExpiresAt || new Date(Date.now() + 7*86400000).toISOString()
        const activatedAt = new Date(Date.now() - (7 - (state?.demoRemainingDays || 7)) * 86400000).toISOString()

        // สร้าง fake activation entry ที่ MyDashboard template ใช้ได้เลย
        const fakeActivation = {
          _id: 'trial-' + (this.user?._id || 'anon'),
          isActive: true,
          activatedAt,
          expiresAt,
          package: {
          ...
          // features (trial = ไม่มี live/qa/miniapps)
          liveEnabled: false,
          qaEnabled: false,
          ...
          atlasEnabled: false,
          longexEnabled: false,
          skill15Enabled: false,
          ixEnabled: false,
          tier: 6,
          extendedDays: 0
        }

        this.activationStore.activations = [fakeActivation]
        // Auto-accept consent for trial (ไม่ต้องกด "ยอมรับ")
        if (!this.activationStore.consentAcceptedIds) this.activationStore.consentAcceptedIds = []
        this.activationStore.consentAcceptedIds.push(fakeActivation._id)
      } catch (err) {
        console.error('[TrialDashboard] load content failed:', err.message)
        this.activationStore.activations = []
      } finally {
        this.activationStore.loading = false
      }
    }
  }
}
