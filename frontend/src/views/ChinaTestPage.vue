...
// ⭐ iOS ต้อง STS (FairPlay DRM) ไม่ใช่ PlayAuth (Ali Prop) — iOS ไม่รองรับ AliyunVoDEncryption
const STS_ENDPOINT = `/api/china/landing-sts/${VIDEO_ID}`
...
// ⭐ SDK v2.35.4 = ground truth (memory project_aliplayer_sdk_version)
//   v2.15.4 พังบน iOS จีน + browser หลายตัว → ห้ามใช้
const ALIPLAYER_VERSIONS = [
  { path: 'self-hosted', v: '2.35.4', jsUrl: '/vendor/aliplayer/aliplayer-2.35.4.js', cssUrl: '/vendor/aliplayer/aliplayer-2.35.4.css' }
]
...
async function fetchSts() {
  ...
  if (!res.ok) throw new Error(`โหลด STS ไม่สำเร็จ (${res.status})`)
  const data = await res.json()
  if (!data.accessKeyId) throw new Error('ไม่ได้ STS credentials')
  return data
}
...
async function initPlayer() {
  try {
    ...
    // ⭐ Root cause iOS หมุนไม่หยุด (code 4010):
    //   "iOS doesn't support VoD's private encryption, please use HLS standard or DRM"
    //   Video มี 2 streams:
    //     - AliyunVoDEncryption (Ali Prop) → Chrome/Android/PC OK, iOS ✗
    //     - Widevine-FairPlay → iOS OK ผ่าน FairPlay
    //   Path per device:
    //     - iOS/Safari → STS + FairPlay stream
    //     - อื่นๆ → PlayAuth + Ali Prop
    const baseConfig = {
      id: 'landing-player',
      vid: VIDEO_ID,
      width: '100%',
    ...
    let playerConfig
    if (isIOS) {
      // iOS/Safari → STS + FairPlay (Widevine-FairPlay stream)
      ...
      // Chrome/Android/PC → PlayAuth + Ali Prop
    ...
    // ⭐ Timeout guard — hang เกิน 8s → แสดง error ให้ user ปิด modal ทันที
    const readyTimeout = setTimeout(() => {
      if (!playerReady.value) {
        errorMsg.value = 'โหลดวิดีโอนานเกินไป กรุณาปิดแล้วลองใหม่'
      }
    }, 8000)
...
.modal-close {
  ...
  position: relative;
  z-index: 9999;  /* ⭐ สูงกว่าทุกอย่าง กัน Aliplayer freeze แล้วปิด modal ไม่ได้ */
}
.modal-close:hover {
