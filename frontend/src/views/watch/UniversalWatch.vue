          ...
          <div v-else-if="video" class="w-player-area" @contextmenu.prevent>
            ...
            <div class="w-video-info">
              ...
              <div class="w-video-actions">
                <button v-if="!isDemo" class="w-ask-btn" :class="{ 'w-ask-cn': region === 'cn' }" @click="_openAsk" :title="msg.askButton">
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      const wantedServ = this.routeRegion === 'cn'
        ? (modeResp.cnMode || modeResp.mode || this.regionConfig.circuitDefault)
        : (modeResp.globalMode || modeResp.mode || this.regionConfig.circuitDefault)
      this.circuitMode = wantedServ === 'ali' ? 'ali' : 'bunny'

      // ⭐ UI (Thai/Chinese) — per-route (admin toggle ได้ override)
      const wantedUi = this.routeRegion === 'cn'
        ? (modeResp.cnUi || 'cn')
        : (modeResp.globalUi || 'global')
      this.region = wantedUi === 'cn' ? 'cn' : 'global'
      this.regionConfig = REGION_MAP[this.region]

      this.country = geoResp.country || ''
    } catch (e) {
      this.circuitMode = this.regionConfig.circuitDefault === 'ali' ? 'ali' : 'bunny'
    }
