...
export const useTrialActivationStore = defineStore('trialActivation', {
  state: () => ({
    activations: [],
    currentSection: null,
    currentVideo: null,
  ...
  actions: {
    async fetchMyActivations() {
      ...
      this.error = null
      try {
        const data = await trialService.getMyActivations()
    ...
    async fetchSection(sectionId) {
      ...
      try {
        const data = await trialService.getSection(sectionId)
    ...
    async fetchVideo(sectionId, videoIndex, bonus = false) {
      ...
      try {
        const data = await trialService.getVideo(sectionId, videoIndex, bonus)
    ...
    async fetchDemoSection() {
      ...
      this.currentSection = null
      try {
        const data = await trialService.getDemoSection()
        this.currentSection = data.section
      } catch (err) {
        this.error = err.response?.data?.message || 'เกิดข้อผิดพลาด'
      } finally {
    ...
    async fetchConsentStatus() {
      try {
        const data = await trialService.getConsentStatus()
    ...
    async acceptConsent(activationId) {
      await trialService.acceptConsent(activationId)
    ...
    async fetchDemoVideo(videoIndex) {
      ...
      this.currentVideo = null
      try {
        const data = await trialService.getDemoVideo(videoIndex)
        this.currentVideo = data.video
      } catch (err) {
        this.error = err.response?.data?.message || 'เกิดข้อผิดพลาด'
      } finally {
