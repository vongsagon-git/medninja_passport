/**
 * 🏆 DDx Arena — Broadcast ประกาศผู้ชนะประจำเดือน
 *
 * ========== แก้ 3 ตัวแปรนี้แล้วรัน ==========
 */
const WINNER_NAME = 'Emmie'
const MONTH = 'พฤษภาคม 2569'
const PRIZE = 'เข้าใช้ระบบ NLEX ฟรี 7 วัน'
const PRIZE_ICON = '📘'
const PRIZE_NOTE = '(นักเรียนปัจจุบัน → รับ VDO BONUS แทน)'
// ==============================================

// mode: 'broadcast' = ส่งทุกคน, 'preview' = ส่งแค่ admin เติ้ล
const MODE = 'preview'

require('dotenv').config({ path: '../.env' })
const { pushMessage, broadcastMessage } = require('./src/modules/line/line.webhook.service')

const ADMIN_UID = 'U2b0de81f0ec73e8561197393683a9e95'

const flexMsg = {
  type: 'flex',
  altText: `🏆 ผู้ชนะ DDx ARENA ${MONTH} — ${WINNER_NAME}!`,
  contents: {
    type: 'bubble',
    size: 'giga',
    header: {
              ...
              contents: [
                {
                  ...
                  cornerRadius: '16px',
                  contents: [
                    { type: 'text', text: PRIZE_ICON, size: 'sm', align: 'center', gravity: 'center' }
                  ],
...
async function run() {
  if (MODE === 'broadcast') {
    ...
    console.log('✅ Broadcast ไปทุกคนแล้ว!')
  } else {
    await pushMessage(ADMIN_UID, [flexMsg])
    console.log('✅ Preview ส่งให้เติ้ลแล้ว — เปลี่ยน MODE = "broadcast" เพื่อส่งจริง')
  }
  process.exit(0)
}
